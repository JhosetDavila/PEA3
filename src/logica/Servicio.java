...
public class Servicio {
    ...
    public Respuesta insertarMulta(Multa multa) {
        ...
        try {
            ...
            Connection con = Conexion.startConeccion();
            String query = "INSERT INTO `sat`.`multa` (`dni`, `tipo_multa`, `monto`, `correo`, `punto`, `fecha`) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public int getCantidadMultasByDNI_Fecha(String dni, Date fecha) {
        try {
            //
            Connection con = Conexion.startConeccion();
            String query = "SELECT COUNT(1) AS cantidad FROM `multa` WHERE dni = ? AND DATE(fecha) = ?";
            PreparedStatement ps = con.prepareStatement(query);
            ps.setString(1, dni);
